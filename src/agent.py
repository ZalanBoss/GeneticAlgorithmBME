...
class Agent():
    def __init__(self, initial_postion, initial_orientation=np.pi/2):
        ...
        self.s2 = [np.array([i,j])*rot_mat(np.pi/4) for (i,j) in self.s1]
        self.s3 = [np.array([i,j])*rot_mat(-np.pi/4) for (i,j) in self.s1]
        self.s4 = [np.array([i,j])*rot_mat(-np.pi/2) for (i,j) in self.s1]
        self.s5 = [np.array([i,j])*rot_mat(np.pi/2) for (i,j) in self.s1]
